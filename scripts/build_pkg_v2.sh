...
# ROI Agent - macOS PKG Common Template Builder
# Creates shared PKG without org-specific config
# Config will be injected via installer script (Datadog-style)

set -e

echo "📦 ROI Agent - PKG Common Template Builder"
echo "==========================================="
...
# 3. .env.template（参照用・フォールバック）
cat > "$RESOURCES_DIR/.env.template" << 'EOF'
# ROI Agent Configuration Template
# This file should be replaced with actual configuration
# 
# DO NOT use this template as-is
# Please run the installer script provided by ROI Dashboard
#
# Required variables:
# ROI_AGENT_BASE_URL=https://roi-service-xxx.run.app/api/v1/device
# ROI_AGENT_API_KEY=YOUR_ORGANIZATION_API_KEY
...
Build Type: Common Package (requires configuration)

INSTALLATION:
  This PKG requires configuration before use.
  Please download the installer from ROI Dashboard:
  https://roi-dashboard-607617540267.asia-northeast1.run.app

MANUAL CONFIGURATION:
  If you installed this PKG directly:
  1. Create .env file: sudo nano /Applications/ROI\ Agent/Resources/.env
  2. Add your organization's API key
  3. Start agent: launchctl bootstrap gui/\$(id -u) ~/Library/LaunchAgents/com.roiagent.plist
...
# 8. postinstallスクリプト（.env未存在時はエージェント起動しない）
cat > "$SCRIPTS_DIR/postinstall" << 'EOF'
#!/bin/bash
# ROI Agent - Post Installation Script
# Common PKG - Does NOT auto-start without configuration
...
if [ ! -f "$ENV_FILE" ]; then
    ...
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""
    echo "This is a common package that requires configuration."
    echo ""
    echo "📥 Recommended: Use the installer from ROI Dashboard"
    echo "   https://roi-dashboard-607617540267.asia-northeast1.run.app"
    echo ""
    echo "🔧 Manual Setup:"
    echo "   1. Create configuration file:"
    echo "      sudo nano /Applications/ROI\\ Agent/Resources/.env"
    echo ""
    echo "   2. Add your organization's API key:"
    echo "      ROI_AGENT_BASE_URL=https://..."
    echo "      ROI_AGENT_API_KEY=YOUR_KEY"
    echo "      ROI_AGENT_INTERVAL_MINUTES=10"
    echo ""
    echo "   3. Start the agent:"
    echo "      launchctl bootstrap gui/\$(id -u) ~/Library/LaunchAgents/com.roiagent.plist"
    echo ""
...
if [ $? -eq 0 ]; then
    echo ""
    echo "✅ Common PKG created successfully!"
    echo ""
    ...
    echo "   gsutil cp $PKG_OUTPUT gs://roi-agent-releases/latest/macos/"
    echo "   gsutil cp $PKG_OUTPUT.sha256 gs://roi-agent-releases/latest/macos/"
    echo ""
